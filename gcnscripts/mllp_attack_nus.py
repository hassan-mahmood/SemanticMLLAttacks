	...
	def test(self):
		...
		#for epoch in range(0,1):
			...
			for current_target_labels in alltargetlabels:
				...
				pgd_store_folder=os.path.join('/mnt/raptor/hassan/cvpr24/attack/nus/',str(model_name),str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				pert_store_folder=os.path.join('/mnt/raptor/hassan/cvpr24/perts/nus/',str(model_name),str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				...
				for jk in range(1,2):
					...
					# pert_image=[]
					# start_idx=0
					# for end_idx in np.linspace(0,newlabels.shape[0],5)[1:]:
					# 	end_idx=int(end_idx)
					...
					pert_image=mllpattack.generate_np(currentinputs,torch.ones_like(newlabels),{'clip_max':1.0,'clip_min':0.0,'y_target':newlabels,'max_iter':10})
