...
class Tester:
	...
	def test(self):
		...
		#for eps_val in [0.006,0.007,0.008,0.009,0.01]:
			...
			target_set_size=3
			max_batch=50
			attacktype=['mlaalpha','mlabeta','gmla','gmlab'][3]
			# attacktype='gmla'

			#alltargetlabels=pickle.load(open(os.path.join('/mnt/raptor/hassan/bmvc/sequences/nusconsistentON/','all'+str(target_set_size)),'rb'))['target_classes']
			...
			for current_target_labels in alltargetlabels:
				...
				#pgd_store_folder=os.path.join('/mnt/raptor/hassan/aaai24/nus/OFFOrthOnly/',str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				pgd_store_folder=os.path.join('/mnt/raptor/hassan/aaai24/supp/voc/ALL/',str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
