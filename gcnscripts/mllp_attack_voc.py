...
class Tester:
	...
	def test(self):
		...
		epoch=0
		#for eps_val in [0.001,0.002,0.003,0.004,0.005,0.006,0.007,0.008,0.009]:
		mllpattack=MLLP(model)
		for eps_val in [0.01]:
		...
		#for epoch in range(0,1):
			...
			target_set_size=3
			max_batch=50
			attacktype=['mlaalpha','mlabeta','gmla','gmlab','mllp'][4]
			# attacktype='gmla'
			...
			# startidx=alltargetlabels.index([16,34])
			for current_target_labels in alltargetlabels:
				...
				for jk in range(1,2):
					...
					# pert_image=mllpattack.generate_np(currentinputs,target_selection_mask,{'clip_max':1.0,'clip_min':0.0,'y_target':newlabels,'max_iter':10})

					pert_image=[]
					start_idx=0
					for end_idx in np.linspace(0,newlabels.shape[0],9)[1:]:
						end_idx=int(end_idx)
						#print(start_idx,end_idx)
						temp_pert_image=mllpattack.generate_np(currentinputs[start_idx:end_idx],torch.ones_like(newlabels[start_idx:end_idx,:]),{'clip_max':1.0,'clip_min':0.0,'y_target':newlabels[start_idx:end_idx,:],'max_iter':10})
						#print(temp_pert_image.shape)
						pert_image.append(torch.clone(temp_pert_image))
						start_idx=end_idx


					pert_image=torch.cat(pert_image,dim=0)
