...
class Tester:
	...
	def test(self):
		...
		if model_name=='gcn':	
			...
			model = gcn_resnet101(num_classes=num_classes, t=0.4, inp_file='MLGCN/data/voc/voc_glove_word2vec.pkl',adj_file='/home/hassan/hassan/Code/MLLAttacks/MLGCN/data/voc/voc_adj.pkl')
		    ...
		    # 'base_model':self.all_base_models[self.model_name],
		    # 'use_weight_norm':self.use_weight_norm,
		    # 'reg_all_layers_norms':self.reg_all_layers_norms
		    # }
			# model=TResnetL(model_params).to(self.training_device)
		...
		epoch=0
		model.eval()
		# for eps_val in [0.01]:#[0.001,0.002,0.003,0.004,0.005,0.006,0.007,0.008,0.009]:
		...
		#for epoch in range(0,1):
			...
			target_set_size=1
			max_batch=50
			# attacktype=['mlaalpha','mlabeta','mllp','gmla'][0]
			# attacktype='gmla'
			...
			for current_target_labels in alltargetlabels:
				print('Source model:',source_model,', Target model:',model_name)
				...
				pgd_store_folder=os.path.join('/mnt/raptor/hassan/cvpr24/attack/voc/',str(source_model)+'_'+str(model_name),str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				pert_store_folder=os.path.join('/mnt/raptor/hassan/cvpr24/perts/voc/',str(source_model),str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				...
				create_folder(pert_store_folder)
				print(pgd_store_folder)
				print(pert_store_folder)
				...
				source_img_perts=os.listdir(pert_store_folder)
				# print(source_img_perts)
				...
				for i, data in enumerate(tqdm(test_dataloader)):
					...
					temp_select_indices=[]
					# print(img_ids)
					for sidx in torch.where(select_indices==True)[0]:
						if img_ids[sidx].replace('images/','')+'.npy' in source_img_perts:
							temp_select_indices.append(sidx)
					#temp_select_indices=torch.where(select_indices==True)[0]
					# print('Selected',len(temp_select_indices))
					...
					# print(img_ids,len(inputs),len(img_ids))
				...
				# get all source perts
				# print(img_ids)
				all_source_perts=[]
				for temp_img_id in img_ids:
					all_source_perts.append(torch.from_numpy(np.load(os.path.join(pert_store_folder,str(temp_img_id.replace('images/',''))+'.npy'))).unsqueeze(dim=0))
				all_source_perts=torch.cat(all_source_perts,dim=0)
				pert_image = torch.clip(inputs + 10*all_source_perts.cuda(),0.0,1.0)


				print('Total number of selected images:',inputs.shape,all_source_perts.shape)
				for jk in range(1,2):
					...
					# tempperts=pert_image-inputs 
					# for pert_idx in range(tempperts.shape[0]):
					# 	np.save(os.path.join(pert_store_folder,str(img_ids[pert_idx].replace('images/','')))+'.npy',tempperts[pert_idx].detach().cpu().numpy())
