...
class Tester:
	...
	def compute_apgd(self,model,inputs,newlabels,mytargetlabels,nontargetlabels, criterion,allotherdata,selection_mask_target):
		apgdt = APGDAttack(model, eps=0.01, norm='Linf', n_iter=300, eot_iter=1, rho=.75, seed=0, device=torch.device('cuda:0'))
	...
	def test(self):
		...
		for epoch in range(0,1):
			...
			target_set_size=1
			max_batch=10
			attacktype=['mlagraph','mla','gmla','mlaproj'][2]
			# attacktype='gmla'
			...
			for current_target_labels in alltargetlabels[1:]:
				...
				for i, data in enumerate(tqdm(test_dataloader)):
					...
					for k in mytargetlabels:
						print('orig labels:',torch.count_nonzero(origlabels[:,k].cpu()==target_on_off))
					...
					#print('Batch inputs size:',batch_inputs.shape)
					if(len(inputs)==1):
						img_ids=[img_ids]

					batch_img_ids+=img_ids

					print('bathc inputs:',batch_inputs.shape)
					del inputs 
