...
class Tester:
	...
	def test(self):
		...
		epoch=0
		#for target_set_size in [3,4,5]:
		# target_set_size=5

		#for eps_val in [0.001,0.002,0.003,0.004,0.005]:
		eps_val=0.05
		#for eps_val in [0.05]:
		for target_set_size in [4,5]:
			...
			max_batch=50
			attacktype=['mlaalpha','mlabeta','gmla','gmlab','gmla_alpha','gmla_beta','gmla_orthonly'][0]
			...
			for current_target_labels in alltargetlabels:
				...
				#current_store_folder=os.path.join('/mnt/raptor/hassan/bmvc/stores/oiBetaOFF_graphonly/','norm_'+str(norm),str(eps_val),str(target_on_off),str(target_set_size),'_'.join([str(i) for i in current_target_labels]))
				#current_store_folder=os.path.join('/mnt/raptor/hassan/bmvc/stores/oiBetaOFFOrthThresh/','norm_'+str(norm),str(eps_val),str(target_on_off),str(target_set_size),'_'.join([str(i) for i in current_target_labels]))
				#
				#current_store_folder=os.path.join('/mnt/raptor/hassan/aaai24/oi/OFFBetaOrthThresh/',str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				current_store_folder=os.path.join('/mnt/raptor/hassan/aaai24/supp/cs/All/',str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				#current_store_folder=os.path.join('/mnt/raptor/hassan/aaai24/oi/OFFGMLAAlpha/',str(eps_val),str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))

				

				orth_store_folder=os.path.join(current_store_folder,'orth_linf','0.85')
				pgd_store_folder=os.path.join(current_store_folder,'pgd_consistent_'+str(attacktype)+'_linf_ON')
