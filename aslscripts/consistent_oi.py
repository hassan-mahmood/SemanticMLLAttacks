...
class Tester:
	...
	def compute_apgd(self,model,inputs,newlabels,mytargetlabels,nontargetlabels, criterion,allotherdata,selection_mask_target):
		...
		apgdt = APGDAttack(model, eps=50 , norm='L2', n_iter=300, eot_iter=1, rho=.75, seed=0, device=torch.device('cuda:0'))
		with torch.no_grad():
	...
	def test(self):
		...
		epoch=0
		for attacktype in ['mlagraph','mla','gmla','mlaproj']:
			...
			target_set_size=1
			max_batch=40
			# attacktype=['mlagraph','mla','gmla','mlaproj'][0]
			# attacktype='gmla'

			

			alltargetlabels=pickle.load(open(os.path.join('/mnt/raptor/hassan/bmvc/sequences/oiconsistentON/','old'+str(target_set_size)),'rb'))
			# print(alltargetlabels)
			alltargetlabels=[[940],[1694],[2788],[5220],[9022],[9321]][::-1]
			for current_target_labels in alltargetlabels:#[63:65]:

				targetlabels=[mapping[t] for t in current_target_labels if t in mapping.keys()]
				#print(current_target_labels,targetlabels)
				...
				pgd_store_folder=os.path.join('/mnt/raptor/hassan/rebut/OFF/',str(target_set_size),'mlaproj','_'.join([str(k) for k in current_target_labels]))
				# pgd_store_folder=os.path.join('/mnt/raptor/hassan/rebut/OFF/',str(target_set_size),attacktype,'_'.join([str(k) for k in current_target_labels]))
				...
				for i, data in enumerate(tqdm(test_dataloader)):
					...
					for jk in range(1,2):
						...
						if attacktype=='gmla' or attacktype=='mlagraph':
							templabels=torch.clone(labels).detach().cpu().numpy()
