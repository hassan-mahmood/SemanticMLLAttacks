...
class Tester:
	...
	def test(self):
		...
		for epoch in range(0,1):
			...
			target_set_size=10
			#alltargetlabels=pickle.load(open(os.path.join('/mnt/raptor/hassan/bmvc/sequences/nus/',str(target_set_size)),'rb'))
			alltargetlabels=[[36,  19,  57,  63,  80,   6]]#16, 20, 26, 47, 73, 80]]
			...
			for current_target_labels in alltargetlabels:
				...
				current_store_folder=os.path.join('/mnt/raptor/hassan/bmvc/stores/nus/','norm_'+str(norm),str(eps_val),str(target_on_off),str(target_set_size),'_'.join([str(i) for i in current_target_labels]))
				

				orth_store_folder=os.path.join(current_store_folder,'orth_linf','0.9')
				pgd_store_folder=os.path.join(current_store_folder,'pgd_linf')
				...
				tempdata.to_hdf(os.path.join(orth_store_folder,'clean.h5'),key='df',mode='w')
				...
				tempdata['ids']=storeimgids
				tempdata.to_hdf(os.path.join(orth_store_folder,'adv.h5'),key='df',mode='w')
				print('Store advoutputs:',storeadvoutputs.shape,tempdata.shape)				



				pickle.dump(storent,open(os.path.join(orth_store_folder,'storent'),'wb'))
				pickle.dump(storefinalindices,open(os.path.join(orth_store_folder,'storefinalindices'),'wb'))
				pickle.dump(storedots,open(os.path.join(orth_store_folder,'storedots'),'wb'))
